...
const useStyles = makeStyles((theme) => ({
  ...
  /*
   * Company list gets its own scroll area when there
   * are many companies. Header and action buttons stay visible.
   */
  companyList: {
    maxHeight: 420,
    overflowY: 'auto',
...
export default function CompanySelector() {
    ...
    const loadCompanies = async () => {
      try {
        ...
        const response = await api.get(
          `/company/${parentCompanyId}/user-assignments/${userId}`,
          {
            headers: {
        ...
        NotificationManager.error(
          'Failed to load company assignments. Please login again.'
        );
  ...
  const handleConfirm = () => {
    ...
    NotificationManager.success(
      `Welcome to ${selectedCompany.companyName}!`
    );
        ...
        <Fade in={true} timeout={800}>
          <Paper className={classes.paper}>
            ...
            <Box className={classes.actionArea}>
              ...
              >
                Continue to{' '}
                {selectedCompany?.companyName || 'Dashboard'}
              </Button>
            </Box>
          </Paper>
        </Fade>
