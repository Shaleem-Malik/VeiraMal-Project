import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { fetchFinanceAnalysis } from "../../Store/Actions/nhtActions";

const FinanceNHTAnalysisWidget = ({ month = "April", data: snapshotData }) => {
  const dispatch = useDispatch();
  const { financeData = [], financeLoading, financeError } = useSelector(
    (state) => state.nht
  );

  useEffect(() => {
    if (!snapshotData) {
      dispatch(fetchFinanceAnalysis("janury"));
    }
  }, [dispatch, snapshotData, month]);

  const finalData = snapshotData || financeData;

  if (!snapshotData && financeLoading) {
    return <p className="m-2">Loading Finance Analysis...</p>;
  }

  if (!snapshotData && financeError) {
    return <p className="m-2 text-danger">Error: {financeError}</p>;
  }

  if (!finalData.length) {
    return <p className="m-2">No Finance Analysis data available.</p>;
  }

  return (
    <div className="card p-3">
      <h3 className="mb-3">Finance Analysis ({month})</h3>
      <div className="table-responsive">
        <table className="table table-bordered table-sm align-middle text-center">
          <thead className="table-light">
            <tr>
              <th rowSpan="2">Unit</th>
              <th colSpan="3">New Hires</th>
              <th colSpan="3">Promotions</th>
              <th rowSpan="2">Int Hire Rate (%)</th>
            </tr>
            <tr>
              <th>Tot (n)</th>
              <th>M (n)</th>
              <th>F (n)</th>
              <th>Tot (n)</th>
              <th>M (n)</th>
              <th>F (n)</th>
            </tr>
          </thead>
          <tbody>
            {finalData.map((row, idx) => (
              <tr key={idx}>
                <td>{row.organizationalUnit ?? "-"}</td>
                <td>{row.newHireTotal ?? 0}</td>
                <td>{row.newHireMale ?? 0}</td>
                <td>{row.newHireFemale ?? 0}</td>
                <td>{row.transferTotal ?? 0}</td>
                <td>{row.transferMale ?? 0}</td>
                <td>{row.transferFemale ?? 0}</td>
                <td>{row.internalHireRate != null ? `${row.internalHireRate}%` : "0%"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default FinanceNHTAnalysisWidget;
